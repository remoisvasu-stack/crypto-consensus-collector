...
# Local storage defaults (HF deployment uses /data via the Dockerfile).
if (-not $env:DATA_DIR)            { $env:DATA_DIR = Join-Path $PSScriptRoot 'data' }
if (-not $env:CONSENSUS_MODEL_DIR) { $env:CONSENSUS_MODEL_DIR = Join-Path $env:DATA_DIR 'consensus_models' }
